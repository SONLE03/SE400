...
data = pd.read_csv('D:/kafka-demo/data_final/display.csv')
    ...
    for i in range(1):
        for j in range(5):
            ...
            if(j == 0):
                ...
                l = Label(text=''+str(data.columns[15]+''), relief=RIDGE, bg=color[i%2], fg=color_f[i%2], pady=15, padx=50, font=10)
            l.grid(row=i+1, column=j, sticky=NSEW)
    for i in range(9):
        for j in range(5):
            # if i % 2 == 0:
            ...
            if(j == 0):
                ...
                l = Label(text=''+str(data.iloc[:,15][i]+''), relief=RIDGE, bg=color[i%2], fg=color_f[i%2], pady=15, padx=50, font=10)
